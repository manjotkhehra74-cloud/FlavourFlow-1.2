...
export const api = {
  ...
  attendanceMe: (month) => request('GET', `/attendance/me${month ? `?month=${month}` : ''}`),
  ...
  leaveRequests: (status) => request('GET', `/leaves?status=${status}`),
  leaveCalendar: (month) => request('GET', `/leaves/calendar?month=${month}`),
